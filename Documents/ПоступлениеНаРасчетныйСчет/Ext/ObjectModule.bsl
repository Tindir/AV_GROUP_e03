...
Процедура СформироватьДвиженияХозрасчетный(ДополнительныеСвойства, Движения, Отказ) Экспорт 
	...
	Запрос.Текст = 
		...
		|	КОНЕЦ КАК ВалютаКт,
		|	&СчетБанк КАК СчетДт,
		|	Документ.СчетОрганизации КАК СубконтоДт1,
		|	Документ.СчетРасчетов КАК СчетКт,
		|	Документ.Контрагент КАК СубконтоКт1,
		|	Документ.Субконто2 КАК СубконтоКт2,
		|	ВЫБОР
		|		КОГДА Документ.Валюта = &ВалютаРегУчета
		...
		|	КОНЕЦ КАК СуммаВал
		|ИЗ
		|	Документ.ПоступлениеНаРасчетныйСчет КАК Документ
		|ГДЕ
	...
	Пока ВДЗ.Следующий() Цикл
		...
		//в БУХ ТУТ универсальная обратка установки собконто. Имитируем
